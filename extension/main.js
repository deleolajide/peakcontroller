  ...
  chrome.hid.getDevices({}, function(devices)
  {
    ...
    WebMidi.enable(function (err)
    {
      if (err) {
        ...
        //if (!output) output = WebMidi.outputs[0];   // no yamaha, just use first one
        ...
        if (WebMidi.outputs.length > 0 && WebMidi.inputs.length > 0)
        {
            ...
            if (input && output)
            {
                console.log("WebMidi devices", input, output);
        ...
        } else console.error("NO MIDI devices available");
      }
...
{   
   if (GuitarCntl.buttonMap.strum.state != 15)  // down or up
   {
      	//console.log("play", chord)
        output.playNote(chord, [1,2], {velocity: 0.5});
        activeChord = chord;
   }

}

function stopChord(tilt)
{
   if (activeChord && GuitarCntl.buttonMap.strum.state == 0)  
   {
   		//console.log("stop", activeChord, tilt, GuitarCntl.buttonMap.strum.state)
        output.stopNote(activeChord, [1,2], {velocity: 0.5});
        activeChord = null;
   }
}
...
function playSectionCheck()
{
  if (!GuitarCntl.buttonMap.yellow.state && !GuitarCntl.buttonMap.blue.state && !GuitarCntl.buttonMap.orange.state && !GuitarCntl.buttonMap.red.state  && !GuitarCntl.buttonMap.green.state)
  {
    ...
    {
        console.debug("advance beat section");
    ...
    {
        console.debug("repeat beat section");
